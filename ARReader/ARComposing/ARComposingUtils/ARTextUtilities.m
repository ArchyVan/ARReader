...
CGAffineTransform ARTextCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from ar_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from ar_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from ar_convertPoint:before[2] toViewOrWindow:to];
    
    return ARTextCGAffineTransformGetFromPoints(before, after);
}
...
UIApplication *ARTextSharedApplication() {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    return ARTextIsAppExtension() ? nil : [UIApplication performSelector:@selector(sharedApplication)];
